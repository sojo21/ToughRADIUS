...
def recharge_card_form(products=[]):
    return pyforms.Form(
        ...
        #修改pyforms.Textbox("batch_no", rules.is_number, maxlength=8, description=u"批次号(年+月+2位序号，如：20150201)",required="required", **input_style),
        pyforms.Textbox("batch_no", rules.is_number, maxlength=2, description=u"批次号(只能是两位数字，如年份：15)", required="required", **input_style),
        pyforms.Dropdown("product_id", args=products, description=u"资费", **input_style),
        #修改pyforms.Textbox("start_no", rules.is_number, maxlength=5, description=u"开始卡号(最大5位)", required="required",**input_style),
        #修改pyforms.Textbox("stop_no", rules.is_number, maxlength=5, description=u"结束卡号(最大5位)", required="required",**input_style),
        pyforms.Textbox("start_no", rules.is_number, maxlength=4, description=u"开始卡号(最大4位)", required="required",**input_style),
        pyforms.Textbox("stop_no", rules.is_number, maxlength=4, description=u"结束卡号(最大4位)", required="required",**input_style),
        pyforms.Textbox("pwd_len", rules.is_number, description=u"密码长度(最大为16)", value=4, **input_style),
        pyforms.Textbox("c_len", rules.is_number, description=u"识别码长度", readonly="readonly", value=2, **input_style),
